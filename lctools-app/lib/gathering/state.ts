 ...
 */
export function createGatheringState(
  node: ResourceNode,
  skill: number = 5,
  expertise: number = 0,
  toolBonus: number = 0
...
export function checkGatheringSuccess(state: GatheringState): GatheringState {
  ...
  newState.luckySuccess =
    newState.currentLP >= newState.luckyPoints && newState.success;
...
export function completeGathering(state: GatheringState): GatheringState {
  ...
  if (newState.success) {
    ...
    newState = logMessage(
      newState,
      "Failed to gather enough materials from the node."
    );
  }
